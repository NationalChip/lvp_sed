...
unsigned char *pvDecoder_InitAmrWbMem(void)
{
    memset(&amrWbDec, 0, sizeof(amrWbDec));
    return (unsigned char *)&amrWbDec;
}
...
DRAM0_STAGE2_SRAM_ATTR int32 pvDecoder_AmrWb(
    int16 prms[],            /* input : parameter vector              */
    int16 synth16k[],        /* output: synthesis speech              */
    int16 * frame_length,    /* output:  lenght of the frame          */
    void *spd_state,         /* i/o   : State structure               */
...
)
{
    ...
    int32 max;
    int16 T0, T0_frac, pit_flag, T0_max, T0_min = 0;
    ...
    *frame_length = AMR_WB_PCM_FRAME;
    ...
    Dpisf_2s_36b(ind, isf, st->past_isfq, st->isf_buf, 1);
