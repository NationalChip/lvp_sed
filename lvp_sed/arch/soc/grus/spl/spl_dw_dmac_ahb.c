...
#include "autoconf.h"
#if defined(CONFIG_ENABLE_SPI_DMA_FOR_FLASH) || defined(CONFIG_DW_UART_DMA) || defined(CONFIG_SPL_DMA_LOAD)
...
void spl_dw_dma_config(void *dst, void *src, int len, int hw_src_per)
{
	...
	spl_clk_set_gate_enable(CLOCK_MODULE_DMA, 1);
...
void spl_dw_dma_start(void)
{
	...
	dw_dma_enable(0);
	spl_clk_set_gate_enable(CLOCK_MODULE_DMA, 0);
}
#endif
